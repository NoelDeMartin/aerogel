<template>
    <Modal ref="$modal">
        <div class="relative flex items-center rounded-md bg-gray-100 p-4 pr-12">
            <SolidAvatar class="mr-2 h-16 w-16 flex-shrink-0" />
        ...
        <div class="px-2">
            <div v-if="$cloud.syncing" class="mt-4 flex items-center gap-2">
                ...
                <Progress v-if="$cloud.syncJob" :job="$cloud.syncJob" bar-class="bg-[--primary-600]" />
            </div>
        ...
        <div v-if="!$cloud.syncing && !$solid.loginOngoing" class="mt-4 flex flex-row-reverse justify-start gap-2">
            <Button v-if="!$solid.isLoggedIn()" @click="$solid.reconnect({ force: true })">
            ...
            <Button color="secondary" @click="$solid.logout()">
...
import { Button, Modal, componentRef, translate, useEvent } from '@aerogel/core';
import type { IModal } from '@aerogel/core';

import Cloud from '@/services/Cloud';

// import UserSettingsModal from './UserSettingsModal.vue';

const $modal = componentRef<IModal>();
